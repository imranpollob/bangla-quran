...
async function main() {
  ...
  if (!suraInput) {
    console.error('Usage: node scripts/compress-audio.js --sura=<number> [--bitrate=56] [--dest=audio/compressed]');
    process.exit(1);
  }
  ...
  for (const task of tasks) {
    ...
    console.log(`✓ ${path.basename(task.input)}: ${(before / 1024).toFixed(1)} KB -> ${(after / 1024).toFixed(1)} KB (${pct}% saved)`);
  }
